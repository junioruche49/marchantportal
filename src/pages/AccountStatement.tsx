...
const AccountStatement = (props: any) => {
  ...
  const { isLoading, error, data } = useQuery("accountStatement", () =>
    getAccountStatementFields(token)
  );

  const {
    isLoading: isProfileSectionLoading,
    error: isProfileSectionError,
    data: profileSectionData,
    refetch,
  } = useQuery("profileSections", () => getProfileSection(token));
  ...
  useEffect(() => {
    profileSectionData &&
      initializeProfileSection(
        [].concat(profileSectionData),
        profileSectionData.every((loan) => loan.completed === true)
      );
  }, [profileSectionData, initializeProfileSection]);
  ...
  const _handleAccountStatementCode = useCallback(
    async (code) => {
      let values = { code: code, purpose: "kyc" };
      setLoading(true);
      try {
        const {
          message,
        ...
        // initializeProfileStore(profileData);
        setTimeout(function () {
          setSubmitted(false);
          setLoading(false);
          refetch();
          pageType === "onboarding"
            ...
            : props.history.replace("/profile/statement_validation");
        }, 2000);
        setLoading(false);
      } catch (error) {
        setLoading(false);
  ...
  const monoConnect = React.useMemo(() => {
    const monoInstance = new MonoConnect({
      onSuccess: async ({ code }) => await _handleAccountStatementCode(code),
  ...
  if (isLoading || isProfileSectionLoading) {
    return (
      <>
  ...
  return (
    <>
      <div className="hidden md:block w-5/12 mb-6 border-r-2 border-gray-200">
        <OnboardingSideBar name="statement_validation" />
        ...
        {submitted ? (
          ...
          <>
            <div className="flex flex-wrap -mx-3 mb-12">
              <div className="w-full m-auto mt-6 mb-6">
                <img
              ...
              <div className="w-full px-3 pt-4">
                <label className="block tracking-wide text-center text-gray-600 text-xs font-semibold mb-10">
                  We work with Mono.co to get your valid bank statement (with
                  your permission of course). Please click the button below to
                  authenticate with Mono before you can continue.
                </label>
